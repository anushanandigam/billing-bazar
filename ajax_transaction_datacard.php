					...
					if($wamount != 0 )
					{
						...
						/*  -----end for retailer--- */
								...
								$sql3=$conn->prepare('update client_wallet set amount=amount+? where add_client_ids=?');
									...
									$sql5 =$conn->prepare('insert into transaction_retailer (`operator_ids`,`datacard_num`,`amount`,`service_ids`,`add_client_ids`,`add_retailer_ids`,`date`) values (?,?,?,?,?,?,?)');

	$sql5->bindParam(1,$operator_ids);
	$sql5->bindParam(2,$datacard_num);
...
if($sql5->execute())
 ...
 $url = "https://api.goprocessing.in/serviceTrans.go?goid=$goid&apikey=$api&rtype=json&msisdn=$datacard_num&operator_code=$operator_ids&amount=$amount&client_trans_id=$client_trans_id&service_family=$sf&apimode=live"; 
...
curl_close($ch);
echo $array = json_decode($result, true);
